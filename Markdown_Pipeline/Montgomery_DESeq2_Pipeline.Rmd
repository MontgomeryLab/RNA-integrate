...
In addition to pdf outputs, Mean Reads Scatter Plots are rendered within the markdown file as html widgets with <strong>draggable zooming, panning, and hover text</strong> features. 
...
If no gene table has been selected/uploaded, only the p-value and fold change thresholds will be customizable. If a gene table has been selected/uploaded, a plot parameters csv 
can be selected from the working directory. 

```{r,include=FALSE,message=FALSE,warning=FALSE}

# Conditional Plot Parameters Import
if (params$plot_parameters_method == "No Table") {plot_parameters = NULL} else {plot_parameters = read.csv(params$plot_parameters)}

# Conditional Nullification of Plot Parameters
if (params$gene_table_method == "No Table" | params$gene_table_method == "Common Names Only") {plot_parameters = NULL}
...
if (is.null(plot_parameters)) {
  ...
  if (params$plot_parameters_method == "Point Colors Only") {
    
    # Point Colors Only Option
    point_color = plot_parameters[,2]
    names(point_color) = plot_parameters[,1]
    point_color = c(point_color,"other" = "#D3D3D3")
    point_size = NULL
    
  } else if (params$plot_parameters_method == "Point Sizes Only") {
    
    # Point Sizes Only Option
    point_color = NULL
    point_size = plot_parameters[,2]
    names(point_size) = plot_parameters[,1]
    point_size = c(point_size,"other" = 0.5)
    
  } else if (params$plot_parameters_method == "Full Table") {
    
    # Full Plot Parameters Option
    point_color = plot_parameters[,2]
    names(point_color) = plot_parameters[,1]
    point_color = c(point_color,"other" = "#D3D3D3")
    point_size = plot_parameters[,3]
    names(point_size) = plot_parameters[,1]
    point_size = c(point_size,"other" = 0.5)
...
mean_reads_input = function(res,group1,group2) {
  ...
  if (is.null(point_color)) {
    ...
    # Customized Point Colors
    color_class = class_names[names(n)]
    color_class = replace(color_class,-which(color_class %in% names(point_size)),"other")
  ...
  if (is.null(point_size)) {
    ...
    size_class = replace(size_class,-which(size_class %in% names(point_size)),"other")
  ...
  mean_reads = ggplot(points_data,aes(x,y)) + 
    ...
    geom_point(aes(color = factor(color_class),size = factor(size_class),alpha = factor(point_sig),
    ...
    scale_alpha_manual(values = c(0.2,0.8),name = "Significance",labels = c("p > 0.05","p < 0.05")) +
...
if (params$generate_mean_reads == TRUE) {
  for (i in 1:length(combinations[1,])){
    pdf(paste0(format(Sys.Date(),format = "%d_%m_%y_"),experiment_id,"_output/",
        ...
        height = 6.5,width = 8)
    print(mean_reads_input(res[[i]],combinations[1,i],combinations[2,i]))
    dev.off()
  }
}
...
for (i in 1:length(combinations[1,])) {
  if (params$generate_mean_reads == TRUE) {
    ...
    mean_reads_out = c(mean_reads_out,knit_expand(text = "```{r mean-reads{{i}},echo=FALSE,warning=FALSE}\n ggplotly(mean_reads_input(res[[{{i}}]],combinations[1,{{i}}],combinations[2,{{i}}]),tooltip = 'text',width = 1000,height = 650)\n```\n"))
  } else {
...
```{r,echo=FALSE,warning=FALSE,fig.height=6.5,fig.width=6.5,results='asis'}
...
if (params$generate_ma == TRUE) {
  ...
  cat("MA Plots not marked for rendering")
}
