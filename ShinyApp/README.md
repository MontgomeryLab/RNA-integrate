...
Welcome to the R Shiny DESeq2 graphics app by the Montgomery Lab at Colorado State University. This pipeline uses tabulated counts files from mRNA or small RNA sequencing experiments to produce differential gene expression (DGE) analysis results and visualizations. In addition to the interactive app interface, the pipeline saves pdf or html files to an output folder within the local directory off the app script as well as a YAML describing the user input parameters used for that experimental run.
...
- Tabulation Software: the tabulation software used to produce counts files or a counts matrix. Options include "Counts Matrix" (FeatureCounts), "tiny RNA" (developed by the Montgomery Lab - https://github.com/MontgomeryLab/tinyRNA), "RSEM", "HTSeq", "Salmon", and "Kallisto". 
  - Counts Matrix: selection from the working directory of the counts matrix file if the Tabulation Software selection is "Counts Matrix" or "tiny RNA".
- Metadata Method: describes whether to use an existing metadata file in the working directory or generate a new metadata file within the app.
  - Metadata: selection from the working directory of the [Metadata CSV](#metadata-csv) if "working directory" is chosen as the metadata method.
  - Condition Number: radio button selection of the number of experimental conditions if "generate new" is chosen as the metadata method.
  - Condition Name, Control Status, and Condition Files: prompts users to enter information about each experimental condition if "generate new" is chosen as the metadata method. 
- Gene Table Method: describes the type of gene table, if any, that will be imported for the experiment. options include "Full Table", "Common Names Only", "Gene Class Only", and "No Table". see [Gene Table](#gene-table).
  - Gene Table: selection from the working directory the gene table, unless "No Table" is chosen as the gene table method.
...
- P-Value Threshold: numeric slider input for classifying significant genes by p-value (between 0.01 and 0.1).
- Fold Change Threshold: numeric slider input for classifying significant genes by fold change (between 1.1 and 2.0).
...
- Customize By Class: checkbox input describing whether mean reads scatter plots should be customized by their classes, as listed in the gene table. Note that a gene table must be imported in order to customize by class. see [Class Parameters CSV](#class-parameters-csv).
  ...
  - Class Selection: selection of classes to plot genes for if the Generate Class Parameters box is checked. Class options are derived from the factor levels of the class column in the gene table.
  - Point Color and Point Size: prompts users to enter information about each selected class if the Generate Class Parameters box is checked.
  - Select Class Parameters: selection from working directory of the class parameters csv, unless the Generate Class Parameters box is checked. If left blank, a default class parameters csv will be generated including all classes listed in the gene table. 
...
- Heatmap Type: describes what type of heatmap is to be rendered and saved. Options include "Complete", "All Classes", and "Selected Slasses". See [Heatmap](#heatmap). If no gene table is uploaded, a complete heatmap will be rendered, and the input widget will not be shown. 
  - Selected Classes: multiple selction list of classes for which heatmaps will be generated if the heatmap type is "Selected Classes". Class options are derived from the factor levels of the class column in the gene table.
