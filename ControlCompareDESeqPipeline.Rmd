...
CompareMA = function(group1,group2){
  ...
  write.csv(resOrdered,paste0(format(Sys.Date(),format = "%d_%m_%y_"),
                              as.character(group1),'vs',as.character(group2),'.csv'))
  pdf(paste0(format(Sys.Date(),format = "%d_%m_%y_"),
             as.character(group1),'vs',as.character(group2),'_MA.pdf'),
      title = paste0(as.character(group1),' vs ',as.character(group2),' MA Plot'))
  plotMA(res, ylim=c(-4,4),main = paste0(group1,' vs ',group2))
  dev.off()
  pdf(paste0(format(Sys.Date(),format = "%d_%m_%y_"),
             as.character(group1),'vs',as.character(group2),'_MA_shrunk.pdf'),
      title = paste0(as.character(group1),' vs ',as.character(group2),' MA Plot (shrunk)'))
...
# Creates table of combinations given an input to specify the control
...
Combinations = CombinationCvR(levels(conditions_set)[1])
...
MA_Files = NULL
for (i in levels(conditions_set)[-1]) {
  MA_Files = c(MA_Files,
               paste0(format(Sys.Date(),format = "%d_%m_%y_"),
                      as.character(levels(conditions_set)[1]),'vs',
                      as.character(i),'_MA.pdf'),
               paste0(format(Sys.Date(),format = "%d_%m_%y_"),
                      as.character(levels(conditions_set)[1]),'vs',
                      as.character(i),'_MA_shrunk.pdf'))
}
...
mean_scatterplot = function(group1,group2){
  ...
  pdf(paste0(format(Sys.Date(),format = "%d_%m_%y_"),group1,'vs',group2,
             "_means_plot.pdf"),title = paste0(group1,' vs ',group2," Means Plot"))
  par(mar=c(5,5,5,2))
